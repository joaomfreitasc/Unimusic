                ...
                artistaDTO.setAlbums(
                    musica
                        ...
                        .map(album -> {
                            ...
                            albumDTO.setDataDeLancamento(
                                album.getDataDeLancamento()
                            );
        ...
        try {
            ...
            if (!Files.exists(arquivo)) {
                throw new RuntimeException(
                    "Arquivo nao encontrado: " + arquivo.toString()
                );
            }
            ...
            if (resource.exists() || resource.isReadable()) {
                return ResponseEntity.ok()
                    .header(
                        ...
                        "inline; arquivo=\"" + resource.getFilename() + "\""
                    )
                    .body(resource);
            } else {
                throw new RuntimeException(
                    "Arquivo nao legivel: " + arquivo.toString()
                );
            }
        } catch (Exception e) {
